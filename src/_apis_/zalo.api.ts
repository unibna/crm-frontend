...
export const getId = async (paramsDefault?: any, endPoint?: string) => {
  try {
    ...
    const url = `/zalo/${endPoint}` + (params.id ? `${params.id}/` : "");
    delete params.id;
...
export const get = async <T>(paramsDefault?: any, endPoint?: string) => {
  try {
    ...
    const result = await APIConfig(baseUrl).get<MultiResponseType<T>>(`/zalo/${endPoint}`, {
      params: paramsUtil,
      cancelToken,
    });
    return result;
