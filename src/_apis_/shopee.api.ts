...
export const getId = async (paramsDefault?: any, endPoint?: string) => {
  try {
    ...
    const url = `/shopee-shop/${endPoint}` + (params.id ? `${params.id}/` : "");
    delete params.id;
...
export const get = async <T>(paramsDefault?: any, endPoint?: string) => {
  try {
    ...
    const result = await APIConfig(baseUrl).get<MultiResponseType<T>>(`/shopee-shop/${endPoint}`, {
      params: paramsUtil,
