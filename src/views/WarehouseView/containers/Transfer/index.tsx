...
const storeTransfer = (state: InitialState, action: any) => {
...
const Transfer = () => {
  ...
  useEffect(() => {
    const newData = data.map((item: any) => {
      return {
        ...item,
        isCheck: transfer.columnSelected.includes(item.id),
      };
    });
  ...
  const loadDataTable = () => {
    const objParams = handleParamsApi(
      {
        ...
        is_confirm: paramsStore.is_confirm_transfer,
        reason: paramsStore.reason_transfer,
        is_deleted: false,
      },
      [
        "search",
    ...
    getListTransfer(objParams);
  };

  const getListTransfer = async (params: any) => {
    if (params) {
      dispatch({
      ...
      if (result && result.data) {
        ...
        const newData = results.map((item: any) => {
          return {
            ...item,
            ...handleDataApi(item, transfer.columnSelected, { color: theme.palette.primary.main }),
          };
        });
  ...
  const handleResizeColumns = (value: any) => {
    dispatchStore({
      type: actionType.RESIZE_COLUMN_TRANSFER,
      payload: {
  ...
  const handleChangeColumnOrder = (columns: string[]) => {
    dispatchStore({
      type: actionType.UPDATE_COLUMN_ORDER_TRANSFER,
      payload: {
  ...
  const handleConfirmSheet = async () => {
    if (!transfer.columnSelected.length) {
      setNotifications({
    ...
    const params = {
      id_list: transfer.columnSelected,
    };
  ...
  const columnShowExport = useMemo(() => {
    return handleDataExport(transfer.resultColumnsShow);
  ...
  const renderHeader = () => {
    ...
    const contentArrButtonOptional: {
      ...
      },
      {
        ...
        handleClick: () =>
          window.open(`/${STATUS_ROLE_WAREHOUSE.SHEET}/new/${STATUS_ROLE_WAREHOUSE.TRANSFER}`),
      },
    ];
    ...
    return (
      <HeaderFilter
        ...
        columnsCount={transfer.countShowColumn}
        originColumns={transfer.columnsShow}
  ...
  const handleCheckColumn = (isCheck: boolean, row: any) => {
    let columnSelected = [...transfer.columnSelected];
    if (isCheck) {
    ...
    dispatchStore({
      type: actionType.UPDATE_COLUMN_SELECTED_TRANSFER,
      payload: {
        columnSelected,
      },
    });
  ...
  const columnOrders = useMemo(() => {
    return transfer.resultColumnsShow.map((item) => item.name);
  }, [transfer.resultColumnsShow]);
  ...
  return (
    <>
      <DDataGrid
        ...
        columns={transfer.resultColumnsShow}
        columnWidths={transfer.columnsWidthResize}
        columnOrders={columnOrders}
        renderHeader={renderHeader}
        listTabDetail={["inventory"]}
        isCheckAll={data.length && data.length === transfer.columnSelected.length}
        contentOptional={{
