...
import {
  actionType,
  summaryColumnFacebookContentIdConversation,
  columnShowFacebookContentIdFilterConversationDetailTargeting,
  summaryColumnFacebookContentIdConversationDetailTargeting,
...
const TopContentIdConversationFacebook = (props: Props) => {
  const {
    isRefresh,
    params: paramsAll = {
  ...
  const { topContentIdFacebookConversation } = store;
  ...
  const loadDataTable = () => {
    const objParams = {
      ...params,
      ...paramsAll,
      objective: objectiveFacebook.CONVERSIONS,
    };
  ...
  const handleResizeColumns = (value: any) => {
    dispatchStore({
      type: actionType.RESIZE_COLUMN_TOP_CONTENT_ID_FACEBOOK_CONVERSATION,
      payload: {
  ...
  const handleChangeColumnOrder = (columns: string[]) => {
    dispatchStore({
      type: actionType.UPDATE_COLUMN_ORDER_TOP_CONTENT_ID_FACEBOOK_CONVERSATION,
      payload: {
    ...
    return (
      <TabWrap value={value} index={0}>
        <TableDetail
          ...
          params={{ ...newParams }}
          columnShowDetail={columnShowFacebookContentIdFilterConversationDetailTargeting}
          summaryDataColumns={summaryColumnFacebookContentIdConversationDetailTargeting}
          contentSummary={{
  ...
  const columnOrders = useMemo(() => {
    return topContentIdFacebookConversation.resultColumnsShow.map((item) => item.name);
  ...
  return (
    <DDataGrid
      ...
      summaryDataColumns={summaryColumnFacebookContentIdConversation}
      columns={topContentIdFacebookConversation.resultColumnsShow}
      columnWidths={topContentIdFacebookConversation.columnsWidthResize}
      ...
      titleHeaderTable="Top chuyển đổi Facebook"
...
export default TopContentIdConversationFacebook;
