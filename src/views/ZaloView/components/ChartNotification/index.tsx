...
const ChartNotification = () => {
  const theme = useTheme();
  const { state: store } = useContext(ZaloContext);
  const { oaFilter } = store;
  const { quota_total = 0, quota_remain = 0 } = oaFilter;

  const dataRender = useMemo(() => {
    return [
      { label: "Đã sử dụng", value: quota_total - quota_remain },
      { label: "Còn", value: quota_remain },
    ];
  }, [oaFilter]);

  const chartSeries = useMemo(() => {
    return fNumber((getObjectPropSafely(() => dataRender[0].value) / quota_total) * 100);
  }, [dataRender]);
  ...
  return (
    <Card>
      <CardHeader title="Hạn mức Notification / tháng" sx={{ mb: 8 }} />
