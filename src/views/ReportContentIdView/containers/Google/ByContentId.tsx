...
import {
  ...
  columnShowContentIdGoogleByDateDetail,
...
const GoogleByContentId = () => {
  ...
  const { [STATUS_ROLE_CONTENT_ID.GOOGLE_BY_CONTENT_ID]: columns, params: paramsStore } = store;
    ...
    return (
      <HeaderFilter
        ...
        onChangeColumn={(columns) =>
          updateCell(STATUS_ROLE_CONTENT_ID.GOOGLE_BY_CONTENT_ID, columns)
        }
  ...
  const renderTableDetail = (row: any, value: number) => {
    const newParams = chooseParams(
      ...
      [...paramsGetDefault, "digital_gg", "customer_id", "effective_status", "objective", "ad_name"]
    );
      ...
      <>
        ...
        <TabWrap value={value} index={1}>
          <TableDetailByDate
            ...
            columns={columnShowContentIdGoogleByDateDetail}
            endpoint="google/date/"
          />
        </TabWrap>
        <TabWrap value={value} index={2}>
          <TableDetailByPhone params={{ ...newParams, type: "GGLADI" }} />
        </TabWrap>
        <TabWrap value={value} index={3}>
          <TableDetailByCampaign
            params={{ ...newParams, ordering: "-cost" }}
            endpoint="google/campaign/"
            columns={columnShowContentIdGoogleCampaignDetail}
          />
        </TabWrap>
      </>
    );
  ...
  const optionFilterChart = useMemo(() => {
    return map(columnShowContentIdGoogle.columnsShowHeader, (item) => ({
      value: item.name,
      label: item.title,
    }));
  }, []);
    ...
    <Grid container spacing={3}>
      ...
      <Grid item xs={12} md={12} lg={12}>
        <DataGrid
          ...
          listTabDetail={["overview", "by_date", "by_phone", "campaign"]}
          ...
          setColumnWidths={(columns) =>
            resizeColumn(STATUS_ROLE_CONTENT_ID.GOOGLE_BY_CONTENT_ID, columns)
          }
          handleChangeColumnOrder={(columns) =>
            orderColumn(STATUS_ROLE_CONTENT_ID.GOOGLE_BY_CONTENT_ID, columns)
          }
        />
      </Grid>
    </Grid>
...
export default GoogleByContentId;
