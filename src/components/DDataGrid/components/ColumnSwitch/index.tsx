...
interface Props {
  ...
  onToggleSwitch: (isActive: boolean, id: string, columnName: string, isLoading: boolean) => void;
...
const ColumnSwitch = (props: Props) => {
  ...
  const Formatter = ({ value, column, row }: { value: any; column: Column; row?: any }) => {
    ...
    const handleSubmit = () => {
      setOpen(false);
      onToggleSwitch(isActiveSwitch, row[keySwitchToggle], column.name, !row.isLoading);
    };
    ...
    const toggleSwitch = (isActive: boolean) => {
      if (row[keySwitchToggle]) {
        if (messagePopupTurnOn || messagePopupTurnOff) {
          ...
          onToggleSwitch(isActive, row[keySwitchToggle], column.name, !row.isLoading);
        }
      }
    };
